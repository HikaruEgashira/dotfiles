...
| AI agent のユーザー設定            | `modules/programs/claude/<file>` (Claude: CLAUDE.md / hooks / statusline 等) と `modules/programs/codex.nix` (Codex: AGENTS.md); Claude `settings.json` は repo 管理せず live `~/.claude/settings.json` が source of truth; MCP は `claude mcp add --scope user` で `~/.claude.json` に登録 |
